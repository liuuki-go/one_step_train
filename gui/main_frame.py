...
from PySide6.QtCore import QThread, Signal,Qt, QEvent, QCoreApplication
...
class MainFrame(QMainWindow):
    ...
    def _set_run_button_loading(self, loading: bool):
        try:
            if loading:
                ...
                self.page_run.btn_run_train.setText(self.tr("训练中"))
            else:
                self.page_run.btn_run_train.setEnabled(True)
                self.page_run.btn_run_train.setText(self.tr("开始训练"))
        except Exception:
            pass
    def _set_one_button_loading(self, loading: bool):
        try:
            if loading:
                ...
                self.page_one.btn_run_one.setText(self.tr("训练中"))
            else:
                self.page_one.btn_run_one.setEnabled(True)
                self.page_one.btn_run_one.setText(self.tr("一键训练"))
        except Exception:
            pass

    def _on_train_done(self, rc: int):
        self._emit_log_to_current(self.tr("训练结束") + f" {rc}")
    ...
    def _on_stop_requested(self):
        ...
        if self.log_thread and self.log_thread.isRunning():
            self._emit_log_to_current(self.tr("正在发送停止信号..."))
            self.log_thread.stop()
        else:
            self._emit_log_to_current(self.tr("当前没有正在运行的训练任务。"))
    ...
    def _init_ui(self):
        ...
        self.lb_function_title = QLabel("功能区")
        self.lb_function_title.setAlignment(Qt.AlignmentFlag.AlignLeft)
        self.lb_function_title.setStyleSheet("QLabel{font-size:20px;font-weight:bold;color:#24292f;margin:0px 0px 0px 5px;}")
        function_title_layout.addWidget(lb_function_title_icon)
        function_title_layout.addWidget(self.lb_function_title)
        ...
        self.lb_tools_title = QLabel("工具区")
        self.lb_tools_title.setAlignment(Qt.AlignmentFlag.AlignLeft)
        self.lb_tools_title.setStyleSheet("QLabel{font-size:20px;font-weight:bold;color:#24292f;margin:0px 0px 0px 5px;}")
        function_tools_layout.addWidget(lb_tools_title_icon)
        function_tools_layout.addWidget(self.lb_tools_title)
        ...
        self.right_container.setStyleSheet("QWidget{background:#F8FAFC;border-right:0px solid #e1e4e8;}")
        ...
        #设置左侧功能区的按钮和样式
        self.btn_one = QPushButton("一键训练"); self.btn_one.setIcon(QIcon(get_resource_path("gui/icon/action_model_icon/one_step_train.png")))
        self.btn_build = QPushButton("构建数据"); self.btn_build.setIcon(QIcon(get_resource_path("gui/icon/action_model_icon/build_dataset.png")))
        self.btn_run = QPushButton("开始训练") ;self.btn_run.setIcon(QIcon(get_resource_path("gui/icon/action_model_icon/run_train.png")))
        ...
        for b in (self.btn_one, self.btn_build, self.btn_run, self.btn_cfg):
            b.setCheckable(True)
            b.setStyleSheet(
        ...
        left_functions_layout.addWidget(self.btn_one)
        left_functions_layout.addWidget(self.btn_build)
        left_functions_layout.addWidget(self.btn_run)
        left_functions_layout.addWidget(self.btn_cfg)
        left_functions_layout.addStretch(1)
        #设置左侧工具区按钮
        
        self.btn_tool_1 = QPushButton("标签处理"); self.btn_tool_1.setIcon(QIcon(get_resource_path("gui/icon/action_model_icon/one_step_train.png")))
        self.btn_tool_2 = QPushButton("模型转换"); self.btn_tool_2.setIcon(QIcon(get_resource_path("gui/icon/action_model_icon/build_dataset.png")))
        for b in (self.btn_tool_1, self.btn_tool_2):
            b.setCheckable(True)
            b.setStyleSheet(
                "QPushButton{font-size:12px;padding:10px 10px;text-align:left;border-radius:10px;}"
                "QPushButton:checked{background:#e6f0ff;color:#0366d6;}"
        ...
        left_tools_layout.addWidget(self.btn_tool_1)
        left_tools_layout.addWidget(self.btn_tool_2)
        ...
        for i, b in enumerate[QPushButton]((self.btn_one, self.btn_build, self.btn_run, self.btn_cfg, self.btn_tool_1, self.btn_tool_2)):  
            action_group.addButton(b, i)

        self.btn_one.setChecked(True) #默认选中一键训练按钮
    ...
    def retranslateUi(self):
        self.setWindowTitle(self.tr("oneST"))
        self.lb_function_title.setText(self.tr("功能区"))
        self.lb_tools_title.setText(self.tr("工具区"))
        self.btn_one.setText(self.tr("一键训练"))
        self.btn_build.setText(self.tr("构建数据"))
        self.btn_run.setText(self.tr("开始训练"))
        self.btn_cfg.setText(self.tr("训练配置"))
        self.btn_tool_1.setText(self.tr("标签处理"))
        self.btn_tool_2.setText(self.tr("模型转换"))
        
        setup_menu(self, self._open_sys_settings, self._set_lang, self.btn_cfg)

        # Dynamic layout adjustment based on language
        lang = LanguageManager.instance().current_lang()
        
        # Calculate optimal width for right monitor area
        right_w = 330
        if hasattr(self, "monitor"):
            self.monitor.retranslateUi()
            right_w = self.monitor.get_optimal_width()
            self.right_container.setMinimumWidth(right_w)
    ...
    def _on_run_requested(self, dataset_root: str, export_path: str):
        ...
        try:
            ...
            if not conda_base:
                QtWidgets.QMessageBox.warning(self, self.tr("警告"), self.tr("Conda 环境路径不能为空。请检查配置文件是否正确或联系管理员。"))
                return
        except Exception:
            QtWidgets.QMessageBox.warning(self, self.tr("警告"), self.tr("获取 Conda 环境路径失败。请检查配置文件是否正确或联系管理员。"))
            return
        ...
        self._emit_log_to_current(self.tr("启动训练"))
        try:
    ...
    def _set_lang(self, lang: str):
        LanguageManager.instance().install(QCoreApplication.instance(), lang)
