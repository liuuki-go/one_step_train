...
class BuildPageWidget(QWidget):
    ...
    def __init__(self):
        ...
        select_label_src_bt = QPushButton("选择文件夹"); select_label_src_bt.clicked.connect(self._pick_src_b)
        ...
        b2 = QPushButton("选择文件"); b2.clicked.connect(self._pick_cls_b)
        ...
        self.lbl_error.setStyleSheet("color: #ff6b6b; font-size: 12px; padding: 5px; background-color: #ffe0e0; border-radius: 4px;")
        self.lbl_error.setVisible(False)  # 初始时隐藏
        form.addWidget(self.lbl_error, 3, 0, 1, 3)  # 跨两列显示
        
        
        #输出数据集路径
        self.lbl_out_b = QLabel("输出数据集路径")
        self.ed_out_b = QLineEdit()
        b3 = QPushButton("选择输出目录"); b3.clicked.connect(self._pick_out_b)
